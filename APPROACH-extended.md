...
ממציא מזהים, שומרים את הקלאסטר המקורי כפי שהוא במקום להשחית את הדאטה. בפועל
הבדיקה משווה בין כל ה-`IDs` שנשלחו ל-`LLM` לבין כל ה-`IDs` שחזרו בתשובה; אם
הקבוצות לא זהות, מתעלמים מהפיצול.
...
| Pass | Input | LLM calls (v1 / v2) | What actually happened |
| ---- | ----- | :-----------------: | ---------------------- |
| `Pass 1` - split clusters | 24 candidate clusters | 24 / 8 | v1 reviewed all 24 clusters. v2 reviewed only the 8 clusters with mixed variant tokens. Both produced ~40 refined clusters. |
| `Pass 2` - match singletons | 17 singletons: 9 original + 8 `Pass 1` orphans | 17 / 9 | Both versions matched all 17 singletons. v2 auto-merged 8 of them by centroid similarity `≥ 0.85`. |
| `Pass 3` - merge clusters | ~30 refined clusters | 32 / 40 | Both versions made 3 real merges: Hebrew/English fragments of `Sony XM5`, `iPhone 16 Pro Max`, and `Xiaomi 14T Pro`. |
| **Total** | | **73 / 57** | **v2 saves 22% of LLM calls without quality loss.** |
